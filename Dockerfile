...
RUN apt-get update
RUN apt-get -y install curl openjdk-7-jre-headless
...
RUN (apt-get install -y build-essential gawk pmount libtool nasm yasm automake cmake gperf zip unzip bison libsdl-dev libsdl-image1.2-dev libsdl-gfx1.2-dev libsdl-mixer1.2-dev libfribidi-dev liblzo2-dev libfreetype6-dev libsqlite3-dev libogg-dev libasound2-dev python-sqlite libglew-dev libcurl3 libcurl4-gnutls-dev libxrandr-dev libxrender-dev libmad0-dev libogg-dev libsmbclient-dev libmysqlclient-dev libpcre3-dev libdbus-1-dev libhal-dev libhal-storage-dev libjasper-dev libfontconfig-dev libbz2-dev libboost-dev libenca-dev libxt-dev libxmu-dev libpng-dev libjpeg-dev libpulse-dev mesa-utils libcdio-dev libsamplerate-dev libmpeg3-dev libflac-dev libiso9660-dev libass-dev libssl-dev fp-compiler gdc libmpeg2-4-dev libmicrohttpd-dev libmodplug-dev gettext cvs python-dev libyajl-dev libboost-thread-dev libplist-dev libtinyxml-dev libcap-dev autopoint libltdl-dev swig libgtk2.0-bin libtag1-dev libtiff-dev libnfs1 libnfs-dev)
...
RUN (cd xbmc && patch -p1 < fixcrash.diff) 

#Configure, make, clean
RUN (cd xbmc && ./bootstrap && ./configure --enable-nfs --enable-upnp --enable-shared-lib --disable-debug --disable-vdpau --disable-vaapi --disable-crystalhd --disable-vdadecoder --disable-vtbdecoder --disable-openmax --disable-joystick  --disable-rsxs --disable-projectm --disable-rtmp --disable-airplay  --disable-afpclient --disable-airtunes --disable-dvdcss --disable-optical-drive --disable-libbluray --disable-ssh --disable-udev --disable-libusb --disable-libcec --disable-libmp3lame --disable-libcap --disable-asap-codec --disable-afpclient --disable-goom --disable-non-free --disable-avahi --disable-texturepacker --disable-libvorbisenc --disable-dbus && make -j2 && cp libxbmc.so /lib && ldconfig && cd xbmc && make -f make_xbmc-server all && mkdir -p /opt/xbmc-server && cp xbmc-server /opt/xbmc-server && cd .. && cp -R addons language media sounds system userdata /opt/xbmc-server/ && cd / && rm -rf /xbmc)

#remove dev packages
#RUN (apt-get remove -y libsdl-dev libsdl-image1.2-dev libsdl-gfx1.2-dev libsdl-mixer1.2-dev libfribidi-dev liblzo2-dev libfreetype6-dev libsqlite3-dev libogg-dev libasound2-dev libglew-dev libcurl3 libcurl4-gnutls-dev libxrender-dev libmad0-dev libogg-dev libsmbclient-dev libmysqlclient-dev libpcre3-dev libdbus-1-dev libhal-dev libhal-storage-dev libjasper-dev libfontconfig-dev libbz2-dev libboost-dev libenca-dev libxt-dev libxmu-dev libpng-dev libjpeg-dev libpulse-dev mesa-utils libcdio-dev libsamplerate-dev libmpeg3-dev libflac-dev libiso9660-dev libass-dev libssl-dev libmpeg2-4-dev libmicrohttpd-dev libmodplug-dev python-dev libyajl-dev libboost-thread-dev libplist-dev libtinyxml-dev libcap-dev libltdl-dev libtag1-dev libtiff-dev libnfs-dev)
